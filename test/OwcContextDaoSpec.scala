...
/**
  * Test Spec for [[models.owc.OwcContextDAO]] with [[OwcContext]] and [[OwcResource]]
  */
class OwcContextDaoSpec extends PlaySpec with OneAppPerTest with BeforeAndAfter with WithTestDatabase {
  ...
  "OwcContext " can {
    "handle OwcResources with DB" in {
      withTestDatabase { database =>
        ...
        val owcOfferingDAO = new OwcOfferingDAO(database, owcPropertiesDAO)
        val owcContextDAO = new OwcContextDAO(database, owcOfferingDAO, owcPropertiesDAO)

        owcContextDAO.getAllOwcResources.size mustEqual 0

        owcContextDAO.createOwcResource(owcResource1) mustEqual Some(owcResource1)
        owcContextDAO.createOwcResource(owcResource2) mustEqual Some(owcResource2)
        owcContextDAO.getAllOwcResources.size mustEqual 2

        val thrown = the[java.sql.SQLException] thrownBy owcContextDAO.createOwcResource(owcResource2)
        thrown.getErrorCode mustEqual 23505

        val entries = owcContextDAO.findOwcResourcesById(owcResource1.id.toString)
        ...
        owcContextDAO.deleteOwcResource(owcResource2) mustEqual true
        owcContextDAO.getAllOwcResources.size mustEqual 1
    ...
    "handle OwcContexts with DB" in {
      withTestDatabase { database =>
        ...
        val thrown2 = the[java.sql.SQLException] thrownBy owcContextDAO.createOwcResource(owcResource2)
    ...
    "handle Users having OwcContexts with DB" in {
      withTestDatabase { database =>
        ...
        owcContextDAO.getAllOwcResources.size mustEqual 70

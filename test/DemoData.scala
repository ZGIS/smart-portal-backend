...
class DemoData {
  ...
  val author1 = OwcAuthor(Some("Alex"))
  val author2 = OwcAuthor(Some("Alex K"), Some(EmailAddress("a.kmoch@gns.cri.nz")))
  val author3 = OwcAuthor(Some("Alex Kmoch"), Some(EmailAddress("a.kmoch@gns.cri.nz")), Some(new URL("http://gns.cri.nz")))
  val author3_1: OwcAuthor = author3.copy(uri = Some(new URL("https://www.gns.cri.nz")))

  val creatorApp1 = OwcCreatorApplication(Some("GW HUB"))
  val creatorApp2 = OwcCreatorApplication(Some("GW HUB"), Some(new URL("http://gns.cri.nz")))
  val creatorApp3 = OwcCreatorApplication(Some("SAC GW HUB"), Some(new URL("http://gns.cri.nz")), Some("v0.9.0"))
  ...
  val category1 = OwcCategory(scheme = Some("view-groups"), term = "sac_add", label = Some("Informative Layers"))
  val category2 = OwcCategory(scheme = "search-domain".toOption(), term = "uncertainty", label = Some("Uncertainty of Models"))
  val category3 = OwcCategory(scheme = "glossary".toOption(), term = "uncertainty", label = Some("margin of error of a measurement"))
  ...
  val style2 = OwcStyleSet(
    ...
    default = Some(true),
    legendUrl = Some(new URL("http://docs.geoserver.org/latest/en/user/_images/line_simpleline1.png")),
    content = Some(owccontent1.newOf)
  )
  ...
  val offering1 = OwcOffering(
    ...
    operations = List(operation1.copy(uuid = UUID.randomUUID()), operation2.newOf),
    contents = List(owccontent1.copy(uuid = UUID.fromString("f811bf08-a158-452b-8c18-3e43c3fdceee"))),
    styles = List(style1.newOf,
      style2.copy(
  ...
  val offering2 = OwcOffering(
    ...
    operations = List(operation3.newOf, operation4.newOf),
    contents = List(owccontent2.copy(uuid = UUID.fromString("f811bf08-a158-452b-8c18-3e43c3fdc000")))
  )

  val offering2_1: OwcOffering = offering2.copy(styles = List(style1.newOf))

  val offering3 = OwcOffering(
    code = OwcOfferingType.WFS.code,
    operations = List(operation5.newOf, operation6.newOf)
  )

  val offering4 = OwcOffering(
    uuid = UUID.randomUUID(),
    code = OwcOfferingType.CSW.code,
    operations = List(operation1.newOf, operation4.newOf),
    contents = List(),
    styles = List()
  )
  ...
  val owcResource1 = OwcResource(
    ...
    preview = List(link3.newOf), // aka links.previews[] and rel=icon (atom)
    contentByRef = List(), // aka links.data[] and rel=enclosure (atom)
    offering = List(offering1.newOf, offering2.newOf),
    active = Some(true),
    resourceMetadata = List(), // aka links.via[] & rel=via
    keyword = List(category1.newOf),
  ...
  val owcResource2 = OwcResource(
    ...
    preview = List(link3.newOf), // aka links.previews[] and rel=icon (atom)
    contentByRef = List(), // aka links.data[] and rel=enclosure (atom)
    offering = List(offering3.newOf, offering4.newOf),
    active = Some(true),
    resourceMetadata = List(), // aka links.via[] & rel=via
    keyword = List(category2.newOf),
    minScaleDenominator = None,
    maxScaleDenominator = None,
  ...
  val owcContext1 = OwcContext(
    ...
    specReference = List(OwcProfile.CORE.newOf), // aka links.profiles[] & rel=profile
    contextMetadata = List(link2.newOf), // aka links.via[] & rel=via
    language = "en",
    ...
    keyword = List(category1.newOf, category2.newOf, category3.newOf),
    resource = List(owcResource1.newOf(new URL("http://portal.smart-project.info/context/resource/smart-sac_copy")),
      owcResource2.newOf(new URL("http://portal.smart-project.info/context/resource/smart-sac-demo_copy"))))
  ...
  val owcContext3: OwcContext = owcContext2.newOf()

...
class CswController @Inject()(implicit configuration: Configuration,
  ...
  /** calls CSW:Transaction -> Insert
    */
  def insert: Action[JsValue] = (authenticationAction andThen userAction).async(parse.json) {
    request =>
            ...
            response._1.xml match {
              case e: Elem if e.label == "TransactionResponse" => {
                ...
                userMetaEntryOk.fold{
                  ...
                  userMetaEntry =>
                    val added = collectionsService.addMdResourceToUserDefaultCollection(CSW_URL, mdMetadata, userMetaEntry)
                    if (added) {
